...
elif CFG.model_name == "UNet_2":
    from model.UNet_2 import UNet_2
    model = UNet_2(in_channels=1, out_channels=1).to(CFG.device)
    print(f"[INFO] Using model: {CFG.model_name}")
...
def train():
    ...
    for epoch in range(CFG.num_epochs):
        ...
        # === Save Timelapse Model ===
        save_name = f"250711_1_epoch_{epoch+1:03d}_dice_{avg_val_dice:.4f}.pth"
        ...
        if CFG.save_best_metric == "dice" and avg_val_dice > best_metric:
            ...
            save_name = f"250711_1_epoch_{epoch+1:03d}_dice_{avg_val_dice:.4f}_best.pth"
            torch.save(model.state_dict(), os.path.join(
                CFG.save_pth_dir, save_name
            ))
            print(f"[INFO] Saved: {save_name}")
